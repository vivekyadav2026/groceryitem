...
<div class="max-w-xl mx-auto bg-white rounded-2xl border border-slate-100 p-6 sm:p-8 shadow-xs">
    ...
    <div class="mb-6 flex items-center justify-between">
        <h3 class="font-serif font-bold text-slate-800 text-lg">Edit Banner</h3>
    ...
    <form method="POST" action="{{ route('admin.banners.update', $banner->id) }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="space-y-1.5">
            ...
            <input type="text" name="title" value="{{ old('title', $banner->title) }}" placeholder="e.g. Elevate Your Aura"
        ...
        <div class="space-y-1.5">
            ...
            <input type="text" name="subtitle" value="{{ old('subtitle', $banner->subtitle) }}" placeholder="e.g. 100% charcoal-free, pure cow dung..."
                   class="w-full border border-slate-200 focus:ring-1 focus:ring-[#C49A6C] focus:border-[#C49A6C] rounded-xl text-sm px-4 py-2.5">
        </div>
        ...
        <div class="space-y-1.5">
            ...
            <input type="text" name="link" value="{{ old('link', $banner->link) }}" placeholder="e.g. /shop"
                   class="w-full border border-slate-200 focus:ring-1 focus:ring-[#C49A6C] focus:border-[#C49A6C] rounded-xl text-sm px-4 py-2.5">
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div class="space-y-1.5">
                ...
                <select name="type" required class="w-full border border-slate-200 focus:ring-1 focus:ring-[#C49A6C] focus:border-[#C49A6C] rounded-xl text-sm px-4 py-2.5 bg-white">
                    <option value="hero" @selected(old('type', $banner->type) === 'hero')>Hero Main Banner</option>
                    <option value="promo" @selected(old('type', $banner->type) === 'promo')>Promo Section</option>
                    <option value="sidebar" @selected(old('type', $banner->type) === 'sidebar')>Sidebar Banner</option>
                </select>
            </div>
        ...
        <!-- Current Image -->
        <div class="space-y-1.5">
            <label class="text-xs font-bold text-slate-500 uppercase tracking-wider block">Current Image</label>
            <img src="{{ asset($banner->image_path) }}" class="h-28 w-60 object-cover rounded-xl border border-slate-100 p-0.5">
        </div>

        <!-- Banner Image -->
        <div class="space-y-1.5">
            <label class="text-xs font-bold text-slate-500 uppercase tracking-wider block">Upload New Image (Optional)</label>
